...
class _MyHomePageState extends State<MyHomePage> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text(widget.title ?? 'Image Picker Example'),
      ),
      body: Center(
...
// 이미지 파일 리스트를 날짜와 위치 정보를 기준으로 분류하여 표시
  Widget displaySortedImages(List<XFile>? fileList) {
    return FutureBuilder<Map<String, List<XFile>>>(
      future: sortImagesByDateAndLocation(fileList),
      builder: (context, snapshot) {
        if (snapshot.connectionState == ConnectionState.done &&
            snapshot.hasData) {
          final sortedImages = snapshot.data!;
          return ListView.builder(
            itemCount: sortedImages.keys.length,
            itemBuilder: (context, index) {
              final key = sortedImages.keys.elementAt(index);

              return ExpansionTile(
                title: Text(key),
                children: sortedImages[key]!
...
// 파일 리스트를 날짜와 위치에 따라 분류
  ...
  Widget _handlePreview() {
    if (isVideo) {
      ...
      return displaySortedImages(_mediaFileList);
    }
  }
